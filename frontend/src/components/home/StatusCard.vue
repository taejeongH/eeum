<template>
  <div class="px-6 mb-10">
    ...
    >
      ...
      <!-- Main Status -->
      <div class="mb-5">
        <p class="text-white/80 text-[11px] font-medium mb-1">
          {{ dependentName || '할머니' }}님의 상태
        </p>
        <h2 class="text-xl font-black leading-tight tracking-tight">"{{ parsedSummary }}"</h2>
      </div>
      ...
      <div class="flex items-center justify-between border-t border-white/20 pt-4">
        ...
        <div
          class="flex items-center bg-white/10 backdrop-blur-md px-3 py-1.5 rounded-xl border border-white/10 group-hover:bg-white/20 transition-all duration-300 leading-none -mr-1"
        >
          <span class="text-[11px] font-black tracking-tight mt-[0.5px]">GMS 상세 분석</span>
          <span
            class="material-symbols-outlined text-[16px] ml-1 -mr-1 opacity-80"
...
const fetchData = async () => {
  try {
    ...
    await familyStore.fetchMembers(familyId);
    ...
    if (currentFamily?.dependentName) {
      ...
      // 2. Fallback: Use store getter
      const dependent = familyStore.dependent;
      if (dependent) {
