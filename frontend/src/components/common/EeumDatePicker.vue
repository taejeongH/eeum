<template>
  <div class="w-full relative">
    ...
    >
      <span
        :class="[
          modelValue ? 'text-slate-800' : 'text-slate-400',
      ...
      <span
        class="material-symbols-outlined text-slate-400 group-hover:text-[#ec856b] transition-colors text-xl flex-shrink-0"
      >
        calendar_today
      </span>
    </div>
        ...
        >
          ...
          >
            ...
            <div class="px-6 py-4 flex justify-between items-center border-b border-slate-50">
              <button
                @click="closePicker"
                class="text-slate-400 hover:text-slate-600 font-medium px-2 py-1 text-sm transition-colors"
              >
                취소
              </button>
              <h3 class="text-base font-bold text-slate-800 select-none">
                {{ isRange ? '기간 선택' : '날짜 선택' }}
              </h3>
              <button
                @click="confirmSelection"
                class="text-[#ec856b] hover:text-[#d46a51] font-bold px-2 py-1 text-sm transition-colors"
              >
                확인
              </button>
            </div>
            ...
            <div class="px-6 relative flex items-center justify-center py-4 mb-2 z-20">
              <!-- 왼쪽: 선택된 연도 표시 -->
              <span
                class="absolute left-6 top-1/2 -translate-y-1/2 text-lg font-bold text-slate-900 select-none"
              >
                {{ currentYear }}년
              </span>

              <!-- 중앙: 이전/다음 월 이동 및 직접 선택 버튼 -->
              <div class="flex items-center gap-2">
                <button
                  @click="prevMonth"
                  class="text-slate-400 hover:text-slate-600 p-1 active:scale-90 transition-transform"
                >
                  <span class="material-symbols-outlined text-3xl">chevron_left</span>
                </button>
                <div
                  @click="toggleWheelPicker"
                  class="flex items-center justify-center cursor-pointer active:scale-95 transition-transform select-none h-10 min-w-[60px]"
                >
                  <h1 class="text-2xl font-bold text-slate-900 leading-none tracking-tight">
                    {{ currentMonth }}월
                  </h1>
                </div>
                <button
            ...
            <div class="relative h-[350px]">
              ...
              <transition name="fade">
                <div
                  v-if="isWheelOpen"
                  class="absolute inset-0 bg-white z-30 flex flex-col items-center justify-center pt-4"
                >
                  <div class="w-full px-8 pb-4">
                    <h4 class="text-center font-bold text-slate-400 mb-4 text-sm">
                      연도와 월을 선택하세요
                    </h4>
                    <div
                      class="relative h-[200px] flex justify-center items-center overflow-hidden"
                    >
                      <div
                      ...
                      <div class="flex w-full z-10 px-4 gap-4">
                        <swiper
                          ...
                          class="h-[200px] flex-1"
                        >
                          <swiper-slide v-for="y in pickerYears" :key="y"
                            ><div class="picker-item">{{ y }}년</div></swiper-slide
                          >
                        </swiper>
                        <swiper
                          :direction="'vertical'"
                        ...
                        >
                          <swiper-slide v-for="m in 12" :key="m"
                            ><div class="picker-item">{{ m }}월</div></swiper-slide
                          >
                        </swiper>
                      </div>
                    </div>
                    <button
                      @click="confirmWheel"
                      class="w-full mt-4 py-3 bg-[#ec856b] text-white rounded-xl font-bold shadow-lg shadow-orange-200 active:scale-95 transition"
                    >
                      이동하기
                    </button>
                  </div>
              ...
              <!-- 달력 그리드 영역 -->
              <div
                class="absolute inset-0 px-5 pb-8 select-none overflow-hidden"
                  ...
                  <div :key="currentMonthKey" class="absolute w-full left-0 px-5 top-[40px]">
                    <div class="grid grid-cols-7 gap-y-1 relative w-full">
                      ...
                      >
                        <!-- 기간 선택 범위 오버레이 -->
                        <div
                          v-if="item.type !== 'empty' && isInRange(item.date)"
                          class="absolute inset-y-0 w-full bg-orange-100/70"
                          :class="{
                            'rounded-l-full': isRangeStart(item.date),
                            'rounded-r-full': isRangeEnd(item.date),
                          }"
                        ></div>
                        <!-- 날짜 선택 버튼 -->
                        <button
...
const calendarDays = computed(() => {
  ...
  for (let i = 1; i <= lastDate; i++)
    days.push({ type: 'current', date: new Date(year, month, i) });
