<template>
  <div
    v-if="emergencyStore.isVisible"
    :class="[
      'fixed inset-0 font-display flex items-center justify-center z-[9999] transition-all duration-300',
      isFullscreen ? 'bg-black p-0' : 'bg-black/80 p-4',
    ]"
  >
    <!-- 전역 응급 상황 모달 컨테이너 -->
    <div
      :class="[
        ...
        isFullscreen
          ? 'w-full h-full max-w-none max-h-none rounded-none bg-black'
          : currentView === 'main'
            ? 'w-full max-w-sm rounded-[32px] min-h-[570px] max-h-[90vh]'
            : 'w-full max-w-2xl rounded-[32px] bg-zinc-950',
      ]"
      :style="
        currentView === 'main'
          ? {
    ...
    >
      <!-- 상단 닫기(오알람 처리) 버튼 - 메인 뷰에서만 노출 -->
      <button
        v-if="currentView === 'main'"
        class="absolute top-4 right-4 z-20 p-2 bg-white/50 backdrop-blur-sm rounded-full hover:bg-white transition-colors animate-[fadeIn_0.3s_ease-out]"
      ...
      <div
        v-if="currentView === 'main'"
        class="flex flex-col h-full w-full bg-white animate-[fadeIn_0.3s_ease-out] overflow-y-auto custom-scrollbar"
      >
        <!-- 상단 위험 알림 헤더 (적색 배경) -->
        <div
          class="bg-red-600 pt-8 pb-6 px-6 text-center flex flex-col items-center relative overflow-hidden shrink-0"
        >
          <div class="relative z-10 flex flex-col items-center gap-2 mb-4">
            <div class="relative">
              <div
                class="relative w-14 h-14 bg-white rounded-full flex items-center justify-center shadow-lg"
              >
          ...
          <div class="relative z-10 flex flex-col items-center">
            <p
              class="text-[10px] font-bold text-red-100 uppercase tracking-widest mb-0.5 opacity-80"
            >
              골든타임 경과
            </p>
            <div
              class="flex items-baseline gap-1 text-white font-black tabular-nums leading-none tracking-tighter"
            >
              <span class="text-6xl">{{ formattedMinutes }}</span>
              <span class="text-3xl opacity-50">:</span>
              <span class="text-6xl animate-[pulse_1s_cubic-bezier(0.4,0,0.6,1)_infinite]">{{
                formattedSeconds
              }}</span>
            </div>
          </div>
          ...
          <div
            v-if="heartRateData"
            class="relative z-10 flex items-center gap-2 mt-4 animate-[fadeIn_0.5s_ease-out_0.2s] bg-white/10 backdrop-blur-sm px-4 py-1.5 rounded-full border border-white/10"
          >
            <svg
              class="w-5 h-5 text-red-300 animate-[pulse_0.8s_ease-in-out_infinite]"
              fill="currentColor"
              viewBox="0 0 24 24"
            >
              <path
            ...
            <span class="text-white font-bold text-lg tabular-nums tracking-tight"
              >{{ heartRateData }} <span class="text-xs font-medium opacity-80">BPM</span></span
            >
          </div>
          <div
            v-else-if="heartRateLoading"
            class="relative z-10 mt-4 flex items-center gap-2 bg-white/5 px-3 py-1 rounded-full"
          >
        ...
        <div class="px-6 py-3 bg-white space-y-2 shrink-0">
          ...
          <!-- 위치 정보 표시 -->
          <div
            v-if="emergencyStore.emergencyData?.location"
        ...
        <div class="px-6 pb-5 pt-2 flex flex-col gap-2 shrink-0 bg-white">
          <div class="grid grid-cols-2 gap-2">
            <!-- 녹화 영상 확인 버튼 -->
            <button
              @click="openVideo('history')"
              class="flex flex-col items-center justify-center gap-1 py-2.5 bg-gray-50 hover:bg-gray-100 text-gray-600 font-bold rounded-xl transition-colors h-16 active:scale-95 shadow-sm border border-gray-100"
            >
            ...
            <button
              ...
              class="flex flex-col items-center justify-center gap-1 py-2.5 bg-gray-50 hover:bg-gray-100 text-gray-900 font-bold rounded-xl border-2 border-red-100 hover:border-red-200 transition-colors h-16 active:scale-95 shadow-sm"
            >
          ...
          <button
            @click="handleFalseAlarm"
            class="text-center text-[11px] font-semibold text-gray-400 underline decoration-gray-300 underline-offset-4 hover:text-gray-600 transition-colors py-1.5"
          >
            오알람으로 처리하고 닫기
          </button>
          <!-- 119 긴급 통화 버튼 -->
          <button
            @click="callEmergency"
            class="w-full py-3.5 bg-gradient-to-r from-red-600 to-red-500 hover:from-red-700 hover:to-red-600 text-white rounded-xl shadow-lg shadow-red-200 active:scale-[0.98] transition-all flex items-center justify-center gap-2 relative overflow-hidden group"
          >
            <div
            ...
            ></div>
            <svg
              class="w-6 h-6 animate-[wiggle_1s_ease-in-out_infinite]"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
      ...
      <!-- [뷰: 비디오] 녹화 영상 또는 실시간 스트리밍 화면 -->
      <div
        v-if="currentView !== 'main'"
        :class="[
          'w-full bg-black flex flex-col animate-[fadeIn_0.3s_ease-out]',
      ...
      >
        ...
        >
          <!-- 닫기 및 뒤로가기 -->
          <button
            @click="closeVideoView"
            class="p-1.5 text-white/80 hover:text-white bg-white/10 rounded-full transition-colors active:scale-90"
            title="영상 닫기"
          >
          ...
          <span class="text-white font-bold text-base tracking-tight">{{
            currentView === 'history' ? '녹화 영상' : '실시간 영상'
          }}</span>
          <div class="w-10"></div>
        </div>
        ...
        <div
          :class="[
            'w-full bg-black relative flex items-center justify-center py-4 min-h-[220px]',
            isFullscreen ? 'flex-1 py-0' : '',
          ]"
        >
          <!-- 비디오/프레임 로드 성공 시 -->
          <div
          ...
          >
            <!-- 실시간 스트리밍 뷰 (WebSocket Image) -->
            <div
              v-if="currentView === 'live'"
              class="relative"
              :class="isFullscreen ? 'h-full w-full flex items-center justify-center' : ''"
            >
              <img
                :src="liveFrameUrl"
            ...
            >
              ...
              >
                <button
                  ...
                  class="p-4 bg-white/20 backdrop-blur-md rounded-full text-white hover:bg-white/30 transition-all active:scale-95 shadow-xl border border-white/20"
                >
        ...
        <div
          ...
          @click.stop
        >
          <!-- 비디오 진행률 및 전체화면 토글 영역 -->
          <div
            v-if="currentView !== 'main'"
            class="bg-gradient-to-t from-zinc-900 to-transparent px-4 pb-1 pt-4 flex flex-col gap-2"
          >
            <!-- 상단 상태 정보 및 버튼 -->
            <div class="flex items-center justify-between">
              <div
                v-if="currentView === 'history'"
                class="flex items-center gap-2 text-[11px] text-white/70 font-mono"
              >
              ...
              <button
                ...
                class="p-1.5 bg-white/10 hover:bg-white/20 rounded-lg text-white/80 transition-all active:scale-95 border border-white/5"
                title="전체화면 전환"
              >
                <svg
                  v-if="!isFullscreen"
            ...
            >
              <div class="h-1.5 w-full bg-white/20 rounded-full overflow-hidden relative">
                <div
                  class="h-full bg-red-600 transition-all duration-75"
          ...
          <div class="px-5 py-4 bg-zinc-900 border-t border-white/5">
            <div class="grid grid-cols-2 gap-3">
              ...
              <button
                @click="handleFalseAlarm"
                class="py-3 bg-white/5 hover:bg-white/10 text-white font-bold rounded-xl border border-white/10 active:scale-[0.98] transition-all flex items-center justify-center gap-2"
              >
              ...
              <button
                @click="callEmergency"
                class="py-3 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl flex items-center justify-center gap-2 shadow-lg active:scale-[0.98] transition-all"
              >
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
                    stroke-linecap="round"
  ...
  render: () =>
    h('svg', { viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', 'stroke-width': '2' }, [
      h('path', {
        'stroke-linecap': 'round',
        'stroke-linejoin': 'round',
        d: 'M13.5 4.5L11 2m0 0l-2.5 2.5M11 2v4m-1 4l-2 3m2-3l1 2 2-3m-3 0V6',
      }),
    ]),
};
const IconHome = {
...
/** 뷰 상태: 'main' (기본 정보), 'history' (녹화 영상), 'live' (실시간 스트리밍) */
...
const toggleFullscreen = () => {
  ...
  showControls.value = true; // 전체화면 전환 시 컨트롤 바 초기화 (보임 처리)

  /** 안드로이드 브릿지를 통한 화면 방향 제어 */
...
const closeVideoView = () => {
  if (isFullscreen.value) {
    toggleFullscreen(); // 화면 방향 및 전체화면 상태 초기화
  }
...
const eventConfig = computed(() => {
  ...
  if (type === 'FALL')
    return { component: IconEmergency, iconColor: 'text-red-500', label: '낙상 감지' };
  if (type === 'OUTING')
    return { component: IconWalk, iconColor: 'text-orange-500', label: '외출 감지' };
  if (type === 'RETURN')
    return { component: IconHome, iconColor: 'text-green-500', label: '귀가 확인' };
...
watch(
  ...
  (visible) => {
    if (visible) {
      ...
      /** 낙상 상황일 경우 심박수 데이터 조회 */
      if (
        emergencyStore.emergencyData?.type === 'FALL' ||
        emergencyStore.emergencyData?.type === 'EMERGENCY'
      ) {
        fetchHeartRate();
      } else {
        /** 초기화 */
...
const fetchHeartRate = async () => {
  ...
  if (!eventId || eventId === 'null' || !familyId || familyId === 'null') {
    Logger.warn('심박수 조회 실패: Event ID 또는 Family ID 누락:', { eventId, familyId });
...
/**
 * 터치 종료 시 드래그 거리에 따라 모달을 닫습니다. (내부 사용)
 */
const onTouchEnd = () => {
