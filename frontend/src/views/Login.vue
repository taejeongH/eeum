  ...
  <div class="min-h-[100dvh] flex items-center justify-center" style="background: linear-gradient(135deg, #ffffff 0%, var(--color-primary-soft) 100%);">
    <div class="w-full max-w-md px-6 py-10">
      <div class="text-center mb-8">
        ...
        <h1 class="eeum-title text-5xl font-extrabold mb-3" style="color: var(--color-primary)">
          이음
        </h1>
      ...
      <form @submit.prevent="handleLogin">
        ...
        <button type="submit" :disabled="isLoading" class="eeum-btn-primary">
...
/**
 * 카카오 소셜 로그인 페이지로 리다이렉트합니다.
 */
const handleKakaoLogin = () => {
  const apiBaseUrl = import.meta.env.VITE_API_BASE_URL || '';
  const KAKAO_LOGIN_URL = `${apiBaseUrl}/api/auth/login/social/kakao`;
...
/**
 * 안드로이드 네이티브 브릿지와 토큰을 공유합니다.
 * @param {string} token
 */
const syncWithAndroidBridge = (token) => {
  if (window.AndroidBridge?.saveAccessToken) {
    window.AndroidBridge.saveAccessToken(token);
  }
};

/**
 * 로그인 에러 발생 시 사용자 메시지를 처리합니다.
 * @param {Error} error
 */
