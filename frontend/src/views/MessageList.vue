<template>
  <div class="min-h-screen bg-gray-50">
    <header class="bg-orange-400 shadow-lg sticky top-0 z-10">
      <div class="max-w-2xl mx-auto px-4 py-6 flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h1 class="text-2xl font-bold text-white">가족 메시지</h1>
    ...
    <main class="max-w-2xl mx-auto px-4 py-6">
      ...
      <div v-else-if="messages.length === 0" class="text-center py-20">
        ...
        <h3 class="text-xl font-semibold text-gray-700 mb-2">No messages sent yet</h3>
        <p class="text-gray-500 mb-6">Share your thoughts with your family</p>
        <button
          @click="goToNewMessage"
          class="px-6 py-3 bg-orange-400 hover:bg-orange-500 text-white font-semibold rounded-full transition-all transform hover:scale-105"
        >
      ...
      <div v-else class="space-y-4">
        <div
          ...
          :key="message.id"
          class="group relative bg-gradient-to-br from-white to-orange-50/30 rounded-3xl p-6 shadow-lg hover:shadow-2xl transition-all duration-500 border border-orange-100/50 hover:border-orange-300/70 hover:-translate-y-2 hover:scale-[1.02]"
        >
          <div class="flex items-start justify-between mb-5">
            <div class="flex items-center gap-4">
              <div class="relative">
                <div class="absolute inset-0 bg-gradient-to-br from-orange-400 to-pink-400 rounded-full blur-md opacity-40 group-hover:opacity-60 transition-opacity"></div>
                <img 
              ...
              <div class="flex-1">
                <div class="flex items-center gap-3 mb-2">
                  <span class="font-bold text-gray-900 text-xl">{{ message.senderRelationship || message.senderName }}</span>
                </div>
            ...
            <div class="flex flex-col items-end gap-2">
              <span
                ...
                :class="message.isRead 
                  ...
                  : 'bg-gradient-to-r from-orange-400 to-pink-400 text-white shadow-lg animate-pulse'"
              >
          ...
          <div class="relative">
            <div class="absolute inset-0 bg-gradient-to-r from-orange-100/50 to-pink-100/50 rounded-2xl blur-sm"></div>
            <div class="relative bg-white/90 backdrop-blur-sm rounded-2xl p-5 border border-orange-100/50 shadow-inner">
              <p class="text-gray-800 whitespace-pre-wrap leading-relaxed text-base font-medium">
                {{ message.content }}
              </p>
            </div>
          </div>

          <div class="flex items-center justify-between mt-4 pt-4 border-t border-orange-100/50">
            <div class="flex items-center gap-4">
              <span class="flex items-center gap-2 text-sm text-gray-600 bg-orange-50 px-3 py-1.5 rounded-full">
                <svg class="w-4 h-4 text-orange-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/>
                </svg>
                메시지
              </span>
              <span v-if="message.enableTTS" class="flex items-center gap-2 text-sm text-orange-600 bg-orange-50 px-3 py-1.5 rounded-full">
      ...
      <div v-if="totalPages > 1" class="mt-8 flex justify-center">
        <div class="flex gap-2">
          <button
            ...
            class="px-4 py-2 bg-white border-2 border-gray-200 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:border-orange-400 transition-colors"
          >
            Previous
          </button>
