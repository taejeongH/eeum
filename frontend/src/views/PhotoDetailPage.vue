    ...
    >
      ...
      <div class="flex items-center gap-2">
        <div v-if="canManage" class="relative" ref="moreMenu">
          ...
          <div
            v-if="showMenu"
            class="absolute right-0 mt-2 w-32 bg-white rounded-xl shadow-xl overflow-hidden z-40 py-1"
          >
            <button
              @click="startEdit"
              class="w-full text-left px-4 py-3 text-[#1c140d] hover:bg-gray-50 text-sm font-medium flex items-center gap-2"
            >
            ...
            <button
              @click="handleDelete"
              class="w-full text-left px-4 py-3 text-red-600 hover:bg-red-50 text-sm font-medium flex items-center gap-2"
            >
...
/**
 * 사진 상세 정보를 가져오고, 이전 페이지의 필터 컨텍스트를 유지하여 전체 목록을 구성합니다.
 */
const fetchPhotoDetail = async () => {
  ...
  try {
    ...
    // 이전 페이지(AlbumPage)의 필터 조건 적용 (업로더, 날짜)
