<template>
  <div @keyup.enter="goNext">
    ...
    <div class="mt-6 mb-8">
      ...
      <p class="mt-2 text-xs text-[var(--color-primary)]">단계 3 / 4 · 비상 연락망 설정</p>
    </div>
    ...
    <div class="grid grid-cols-3 gap-3 mb-8">
      <div
        ...
        :class="[
          contactSlots[index]
            ? 'border-[var(--color-primary)] bg-[var(--color-primary-soft)] border-solid'
      ...
      >
        <div
          class="absolute top-2 left-2 w-6 h-6 rounded-full bg-white flex items-center justify-center text-xs font-bold text-gray-500 shadow-sm"
        ...
        <template v-if="contactSlots[index]">
          <div class="w-12 h-12 rounded-full mb-2 bg-white p-0.5 shadow-sm">
            <img
              :src="contactSlots[index].profileImage || '/default-profile.png'"
        ...
        <template v-else>
          <div
            class="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-gray-300 mb-2"
          >
    ...
    <div class="bg-white rounded-3xl shadow-lg border border-gray-100 p-5 mb-8">
      ...
      <div class="grid grid-cols-4 gap-3">
        <template v-for="member in availableMembers" :key="member.userId">
          ...
          >
            <div
              class="relative w-14 h-14 rounded-full p-0.5 bg-white ring-1 ring-gray-200 transition-all group-hover:ring-gray-300 group-disabled:ring-gray-100"
            >
              <img
            ...
            <span class="text-xs font-medium text-gray-600 truncate w-full text-center">{{
              member.name
            }}</span>
          </button>
        </template>
        ...
        <div
          ...
          class="col-span-4 py-4 text-center text-xs text-gray-400 bg-gray-50 rounded-xl"
        >
          선택 가능한 멤버가 없습니다.
        </div>
      </div>
    ...
    <!-- 하단 버튼 영역 -->
    <div class="mt-10">
      <button @click="goNext" class="eeum-btn-primary">다음 단계로 →</button>
    </div>
  </div>
</template>
...
/**
 * 연락처 등록 후 다음 빈 슬롯으로 자동 선택 이동합니다.
 */
const autoAdvanceSlot = () => {
