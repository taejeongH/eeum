<template>
  <div class="bg-[#fcfcfc] min-h-screen pb-20">
    <div
      class="bg-white/90 backdrop-blur-md sticky top-0 z-[90] border-b border-gray-100 shadow-sm px-4 py-4 flex items-center gap-3 transition-all duration-300"
    >
      <button
        @click="router.back()"
        class="p-2 -ml-1 rounded-full hover:bg-gray-100 transition-colors text-gray-600"
      >
        <svg
    ...
    <main class="px-5 py-6 space-y-8">
      ...
      <section v-if="isRepresentative" class="space-y-4">
        ...
        <div class="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 transition-all">
          ...
          <div v-else class="flex flex-col items-center">
            <div
              class="qr-container bg-white p-4 rounded-2xl border border-gray-100 shadow-sm mb-6"
            >
            ...
            <div class="text-center mb-6">
              ...
              <p
                ...
                :class="remainingTime <= 60 ? 'text-red-500' : 'text-gray-400'"
              >
                {{ expiryText }}
              </p>
            </div>
      ...
      <section class="space-y-4">
        ...
        <div v-if="isLoadingDevices" class="flex flex-col items-center justify-center py-10">
          ...
          ></div>
          <p class="mt-4 text-gray-400 text-xs font-bold uppercase tracking-widest">
            데이터를 불러오고 있습니다
          </p>
        </div>
        ...
        <div
          ...
          class="bg-white rounded-3xl p-10 border border-dashed border-gray-200 flex flex-col items-center text-center"
        >
          <div class="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center mb-4">
            <svg
              xmlns="http://www.w3.org/2000/svg"
        ...
        <div v-else class="space-y-4">
          <div
            ...
            :key="device.id"
            class="bg-white rounded-3xl p-5 shadow-sm border border-gray-100 flex items-center gap-4 group transition-all relative overflow-hidden"
          >
            <!-- 기기 아이콘 -->
            <div
              class="w-14 h-14 rounded-2xl shrink-0 flex items-center justify-center transition-colors"
              :class="
                device.isActive
                  ? 'bg-[var(--color-primary-soft)] text-[var(--color-primary-light)]'
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-0.5">
                <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{{
                  getLocationName(device.locationType)
                }}</span>
              </div>
              <h3 class="text-lg font-bold text-gray-900 truncate tracking-tight">
                {{ device.deviceName }}
              </h3>
              <p class="text-[11px] font-bold text-gray-400 mt-0.5 uppercase tracking-widest">
            ...
            <div v-if="isRepresentative" class="flex flex-col gap-2">
              <button
                ...
                class="w-8 h-8 rounded-full bg-gray-50 flex items-center justify-center text-gray-400 hover:bg-orange-50 hover:text-[var(--color-primary)] transition-all"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-4 w-4"
              ...
              <button
                ...
                class="w-8 h-8 rounded-full bg-gray-50 flex items-center justify-center text-gray-300 hover:bg-red-50 hover:text-red-500 transition-all"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-4 w-4"
    ...
    <transition name="modal">
      <div
        ...
        class="fixed inset-0 z-[200] flex items-end sm:items-center justify-center p-0 sm:p-6"
      >
        <!-- 배경 어두움 처리 -->
        <div
        ...
        >
          ...
          <div v-if="showEditModal">
            ...
            <div class="space-y-6 mb-10">
              <div class="space-y-2">
                <label class="text-[11px] font-bold text-gray-400 uppercase tracking-widest ml-1"
                  >기기 이름</label
                >
                <input
              ...
              <div class="space-y-2">
                ...
                <div class="grid grid-cols-2 gap-2">
                  <button
                    ...
                    :class="
                      editForm.locationType === loc.value
            ...
            <div class="flex gap-3">
              <button
                ...
                class="flex-1 h-14 rounded-[var(--radius-xl)] bg-gray-50 text-gray-400 font-bold hover:bg-gray-100 transition-all active:scale-95"
              >
                취소
              </button>
              <button
                @click="saveDevice"
                :disabled="isSaving"
                class="flex-1 eeum-btn-primary flex items-center justify-center gap-2"
              >
                <div
                  v-if="isSaving"
                  class="animate-spin rounded-full h-4 w-4 border-b-2 border-white"
          ...
          <div v-if="showDeleteModal">
            <div
              class="w-20 h-20 bg-red-50 rounded-3xl flex items-center justify-center mb-6 mx-auto"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
            ...
            <h3 class="text-2xl font-black text-gray-900 mb-2 tracking-tight text-center">
              기기 삭제
            </h3>
            <p class="text-sm text-gray-500 mb-8 text-center leading-relaxed">
              <span class="text-gray-900 font-black">'{{ deviceToDelete?.deviceName }}'</span
              ><br />기기를 정말로 삭제하시겠습니까?
            </p>
            ...
            <div class="flex gap-3">
              <button
                ...
                class="flex-1 h-14 rounded-[var(--radius-xl)] bg-gray-50 text-gray-400 font-bold transition-all active:scale-95"
              >
                취소
              </button>
              <button
                @click="deleteDevice"
                :disabled="isDeleting"
                class="flex-1 h-14 rounded-[var(--radius-xl)] bg-red-500 text-white font-bold active:scale-95 transition-all flex items-center justify-center gap-2"
              >
                <div
                  v-if="isDeleting"
                  class="animate-spin rounded-full h-4 w-4 border-b-2 border-white"
...
const generateQR = async () => {
  ...
  try {
    ...
    alert(
      `QR 코드 생성 실패: ${error.response?.data?.message || error.message || JSON.stringify(error)}`,
    );
  } finally {
