  ...
  <div class="min-h-screen flex items-center justify-center" style="background: linear-gradient(135deg, #ffffff 0%, var(--color-primary-soft) 100%);">
    <div class="w-full max-w-md px-6 py-10">
      ...
      <div class="text-center mb-8">
        <h1 class="text-3xl font-extrabold mb-2" style="color: var(--color-primary);">계정/비밀번호 찾기</h1>
      ...
      <div class="bg-white p-8 rounded-2xl shadow-xl border border-gray-100">
        ...
        <div class="flex border-b border-gray-200 mb-6">
          ...
          >
            이메일 찾기
            <div v-if="activeTab === 'findEmail'" class="absolute bottom-0 left-0 w-full h-0.5" style="background-color: var(--color-primary);"></div>
          </button>
          <button 
            @click="activeTab = 'resetPassword'"
            class="flex-1 pb-3 text-sm font-medium transition-colors relative"
            :class="activeTab === 'resetPassword' ? 'font-bold' : 'text-gray-400 hover:text-gray-600'"
            :style="activeTab === 'resetPassword' ? { color: 'var(--color-primary)' } : {}"
          >
            비밀번호 재설정
            <div v-if="activeTab === 'resetPassword'" class="absolute bottom-0 left-0 w-full h-0.5" style="background-color: var(--color-primary);"></div>
          </button>
        </div>
        ...
        <div v-if="activeTab === 'findEmail'" class="animate-fade-in">
          ...
          <!-- Result -->
          <div v-if="foundEmail" class="mt-6 p-4 bg-orange-50 rounded-xl border border-orange-100 text-center animate-fade-in">
        ...
        <div v-if="activeTab === 'resetPassword'" class="animate-fade-in">
          ...
          <div v-else>
              ...
              <div>
                ...
                <div class="flex gap-2">
                  ...
                  <button 
                    ...
                    :style="!codeSent ? { backgroundColor: 'var(--color-primary-soft)', color: 'var(--color-primary)' } : {}"
                  >
                    {{ codeSent ? '전송 완료' : '전송' }}
                  </button>
                </div>
            ...
            <!-- Step 2: New Password -->
            <form v-else-if="resetStep === 2" @submit.prevent="handleResetPassword" class="space-y-4">
              <div>
        ...
        <div v-if="message && !resetSuccess" class="mt-4 text-center text-sm font-medium bg-red-50 p-3 rounded-lg text-red-600 animate-pulse">
            {{ message }}
        </div>
      </div>

      <div class="text-center mt-6">
        <button @click="$router.push('/login')" class="text-gray-500 text-sm hover:text-gray-700 font-medium">
          로그인 페이지로 돌아가기
        </button>
      </div>
    </div>
...
import { findEmail, sendPasswordResetCode, verifyPasswordResetCode, resetPassword } from '@/services/api';
